...
export class APIPromise<T> extends Promise<T> {
   ...
   * or add one of these imports before your first `import … from 'train-travel-friction-analysis-demo'`:
  ...
  protected stringifyQuery(query: Record<string, unknown>): string {
    return Object.entries(query)
      ...
      .map(([key, value]) => {
        ...
        throw new TrainTravelFrictionAnalysisError(
...
const validatePositiveInteger = (name: string, n: unknown): number => {
  if (typeof n !== 'number' || !Number.isInteger(n)) {
    throw new TrainTravelFrictionAnalysisError(`${name} must be an integer`);
  }
  if (n < 0) {
    throw new TrainTravelFrictionAnalysisError(`${name} must be a positive integer`);
  }
  return n;
};
...
export const ensurePresent = <T>(value: T | null | undefined): T => {
  if (value == null)
    throw new TrainTravelFrictionAnalysisError(`Expected a value to be given but received ${value} instead.`);
  return value;
};
...
export function debug(action: string, ...args: any[]) {
  if (typeof process !== 'undefined' && process?.env?.['DEBUG'] === 'true') {
    console.log(`TrainTravelFrictionAnalysis:DEBUG:${action}`, ...args);
  }
}
